...
export default function GenresPage() {
  ...
  const filtered = activeGenre
    ? films.filter((f) => f.genre.toLowerCase() === activeGenre.toLowerCase())
    : films;
  ...
  return (
    <main className="min-h-screen bg-background text-white">
      ...
      <div className="px-4 sm:px-8 py-8">
        <h1 className="text-2xl sm:text-4xl font-black tracking-tight mb-6">Genres</h1>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs sm:text-sm font-semibold transition ${
              activeGenre === null
                ...
                : "bg-surface text-gray-300 hover:bg-white/20"
            }`}
          >
            All
          </button>
        ...
        {loading ? (
          ...
          <p className="text-gray-400">
            No films found{activeGenre ? ` in ${activeGenre}` : ""}.
          </p>
        ) : (
