...
export default function NewFilmPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.trailer_url) {
      setError("Please upload a trailer video first.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(typeof data.detail === "string" ? data.detail : "Failed to create film");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-2xl">
      <h1 className="text-xl sm:text-2xl font-bold mb-5 sm:mb-6">Add New Film</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            >
              {posterBusy ? "Uploading..." : posterRetry ? "Retry" : "Upload"}
            </button>
          </div>
        ...
        <div>
          <label className={labelClass}>Backdrop Image (1920×1080px, landscape — for the homepage hero)</label>
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <input
              type="file"
              accept=".jpg,.jpeg,.png,.webp"
            ...
            <button
              ...
              className="bg-white/20 px-3 py-1.5 rounded text-xs font-semibold disabled:opacity-50 whitespace-nowrap shrink-0"
            >
              {backdropBusy ? "Uploading..." : backdropRetry ? "Retry" : "Upload"}
            </button>
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="bg-white/20 px-3 py-1.5 rounded text-xs font-semibold disabled:opacity-50 whitespace-nowrap shrink-0"
            >
              {videoBusy ? "Uploading..." : videoRetry ? "Retry" : "Upload"}
            </button>
          </div>
          <UploadProgressBar state={videoState} />
          {form.video_url && (
            <p className="text-xs text-gray-500 mt-1 truncate">Saved: {form.video_url}</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="bg-white/20 px-3 py-1.5 rounded text-xs font-semibold disabled:opacity-50 whitespace-nowrap shrink-0"
            >
              {trailerBusy ? "Uploading..." : trailerRetry ? "Retry" : "Upload"}
            </button>
          </div>
          <UploadProgressBar state={trailerState} />
          {form.trailer_url && (
            <p className="text-xs text-gray-500 mt-1 truncate">Saved: {form.trailer_url}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            The minimum a non-subscriber must pay to unlock this film via VIP Pass. Defaults to ₦1,000.
          </p>
        </div>
